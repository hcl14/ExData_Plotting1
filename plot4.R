...
par(mfrow=c(2,2), mar=c(2.8,2.8,4,2))

## If the weekdays are not in English do Sys.setlocale("LC_TIME", "C")
plot(m[,2], m$Global_active_power, type="l", xlab = "",
     ylab="Global Active Power",cex.lab=0.8, cex.axis=0.8, cex.main = 0.9, mgp = c(1.8, 0.8, 0))
## axis labes and main are a little bit smaller; labels are closer to plot

plot(m[,2], m$Voltage, type="l", xlab = "datetime",
     ylab="Voltage",cex.lab=0.8, cex.axis=0.8, cex.main = 0.9, mgp = c(1.8, 0.8, 0))


plot(m[,2], m[,7], type="l", xlab = "",
     ylab="Energy sub metering", mgp = c(1.8, 0.8, 0), cex.lab=0.8, cex.axis=0.8, cex.main = 0.9, col="black")
lines(m[,2],m[,8],col="red")
lines(m[,2],m[,9],col="blue")
legend("topright", col=c("black","red","blue"), legend = colnames(m)[7:9], lty=c(1.1,1.1), cex = 0.5, bty = "n", adj=-0.2)

plot(m[,2], m$Global_reactive_power, type="l", xlab = "datetime",
     ylab="Global_reactive_power",cex.lab=0.8, cex.axis=0.8, cex.main = 0.9, mgp = c(1.8, 0.8, 0))
## axis labes and main are a little bit smaller; labels are closer to plot
